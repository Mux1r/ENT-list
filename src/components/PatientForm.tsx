...
export default function PatientForm({ patient, onSubmit, onCancel }: PatientFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ ...formData as Patient, id: patient?.id || Math.random().toString(36).substring(7) });
  };
        ...
        <div className="bg-sage-600 p-6 text-white flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-sage-100 font-bold uppercase tracking-widest mt-1">Patient Registration</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input type="text" value={formData.bedNumber} onChange={(e) => updateField('bedNumber', e.target.value)}
            ...
            <div>
              ...
              <input type="number" value={formData.age} onChange={(e) => updateField('age', parseInt(e.target.value))}
                className="w-full px-4 py-2.5 bg-natural-50 border border-natural-200 rounded-lg text-sm focus:ring-2 focus:ring-sage-500/20 focus:border-sage-500 outline-hidden font-bold transition-all text-center" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div>
              ...
              <select value={formData.gender} onChange={(e) => updateField('gender', e.target.value as Gender)}
                className="w-full px-4 py-2.5 bg-natural-50 border border-natural-200 rounded-lg text-sm focus:ring-2 focus:ring-sage-500/20 focus:border-sage-500 outline-hidden font-bold transition-all appearance-none cursor-pointer">
            ...
            <div>
              ...
              <input type="text" value={formData.chartNumber} onChange={(e) => updateField('chartNumber', e.target.value)}
                className="w-full px-4 py-2.5 bg-natural-50 border border-natural-200 rounded-lg text-sm focus:ring-2 focus:ring-sage-500/20 focus:border-sage-500 outline-hidden font-bold transition-all"
                placeholder="1234567" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-natural-400 uppercase tracking-widest mb-2 px-1">入住日 Adm. Date</label>
          ...
          <div>
            <label className="block text-[10px] font-bold text-natural-400 uppercase tracking-widest mb-2 px-1">診斷 Diagnosis</label>
            <textarea rows={3} value={formData.diagnosis} onChange={(e) => updateField('diagnosis', e.target.value)}
              className="w-full px-4 py-3 bg-natural-50 border border-natural-200 rounded-xl text-sm focus:ring-2 focus:ring-sage-500/20 focus:border-sage-500 outline-hidden transition-all resize-none font-medium text-natural-800 placeholder-natural-300"
              placeholder="例: Laryngeal cancer, cT3N1M0, S/P Total Laryngectomy..." />
          </div>
